...
import com.hololivefinder.api.YoutubeDataApi;
import com.hololivefinder.db.DBController;
import com.hololivefinder.model.data.Member;
import com.hololivefinder.model.live.LiveModel;
import com.hololivefinder.model.profile.Item;
import com.hololivefinder.model.profile.ProfileModel;
...
public class DataManagement {
	...
	public void LiveConfirm() {
		...
		try {//web페이지를 파싱해서 live여부를 확인 할당량을 아끼기위해 사용함
			...
			if(liveids!="") {
				...
				ObjectMapper mapper = new ObjectMapper();
				List<com.hololivefinder.model.live.Item> items = mapper.readValue(jsonString, LiveModel.class).getItems();
				for(int i=0;i<items.size();i++) {
					...
					if(items.get(i).getSnippet().getLiveBroadcastContent().equals("live")) {
						...
						list.get(index).setOnAirTitle(items.get(i).getSnippet().getChannelTitle());
	...
	private int getIndex(com.hololivefinder.model.live.Item item) {//api에서 불러온 값이 순서대로가 아니라 순서를 확인해줄 index
		for(int i=0;i<memberNameT.length;i++) {
